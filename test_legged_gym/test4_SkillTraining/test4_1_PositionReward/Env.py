...
class Env:
    ...
    def _init_buffers(self):
        ...
        self.target_pos = torch.zeros(self.num_envs, 3, device=self.device, requires_grad=False)
    ...
    def _resample_commands(self, env_ids):
        ...
        if(len(env_ids)==0):
            return
        self.commands[env_ids, 0] = torch_rand_float(self.commands_ranges["x_dist"][0], self.commands_ranges["x_dist"][1], (len(env_ids), 1), device=self.device).squeeze(1)
        self.commands[env_ids, 1] = torch_rand_float(self.commands_ranges["y_dist"][0], self.commands_ranges["y_dist"][1], (len(env_ids), 1), device=self.device).squeeze(1)
        self.commands[env_ids, 2] = torch_rand_float(self.commands_ranges["heading"][0], self.commands_ranges["heading"][1], (len(env_ids), 1), device=self.device).squeeze(1)
    ...
    def apply_command_position(self, env_ids, target_pos):
        ...
        Args:
            ...
            pos (torch.Tensor or np.ndarray): Target positions with shape (1, 3)
    ...
    def _draw_debug_vis(self):
        ...
        if self.target_pos is not None:
            self.sphere_geoms_red.draw(torch.tensor([self.target_pos[0][0],self.target_pos[0][1],self.base_pos[0][2]]).to(self.device), self.gym, self.viewer, self.envs[0])
